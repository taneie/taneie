...
- スマホ、タブレット、PCで主要導線と表示情報を揃える。
- LPヘッダーとログイン後ヘッダーはスクロール中も操作できる固定ヘッダーとする。
- ログイン後ヘッダーの高さは実測してCSS変数に反映し、本文が端末幅ごとのヘッダー高さに隠れないようにする。
- ナビゲーションは端末幅で非表示にせず、横スクロールで同一項目を維持する。
- 端末別の専用UIは最小限にし、カードやフォームは同じ構成のまま折り返しで調整する。
- Windows / macOS / iOS / Android での見え方を揃えるため、グローバルCSSでフォント、フォーム外観、safe-area、フォーカス表示を標準化する。
