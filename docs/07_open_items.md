...
| OPN-002 | レジュメファイル保存 | 本番はGCS保存、開発は未設定でも許容。削除・スキャンは未実装 | 削除方針、ウイルススキャン、保存期限を設計 |
| OPN-003 | 本番インフラ         | Cloud Run + Neon + GCSの運用手順は `06_operations.md` に集約済み | 監視、バックアップ、アラート、障害対応を具体化 |
...
| OPN-006 | メール通知           | 稼働確認のみ実装済み。詳細は `09_email_notification_flows.md` | パスワード再設定、応募、面談、スカウト、問い合わせ通知を優先実装 |
...
| 運用補足手順               | 障害対応、DBバックアップ、鍵ローテーション、監視、アラート        |
